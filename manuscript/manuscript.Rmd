...
cap <- paste0(
  "\\label{fig:decreased-normality}",
  "Power transformations may reduce normality depending on location. ",
  "Variable $x$ consists of $10000$ values randomly drawn from a normal distribution $\\mathcal{N}(\\mu, 1)$. ",
  ...
  "and even breaks down due to numerical issues for $\\mu > 5 \\cdot 10^4$.")
...
-   We define an empirical goodness of fit test for detecting cases where power transformations fail to yield a normally distributed transformed feature.
...
Box-Cox and Yeo-Johnson transformations are modified by introducing a shift parameter $x_0$ into equations \ref{eqn:box-cox-original} and \ref{eqn:yeo-johnson-original}. The shift-sensitive Box-Cox transformation of a feature value $x_i$ of feature $\mathbf{X}$ under transformation parameter $\lambda$ and shift parameter $x_0$ is then:
...
```{=latex}
\begin{equation}
\label{eqn:box-cox-shift-log-likelihood}
...
```{=latex}
\begin{equation}
\label{eqn:yeo-johnson-shift-log-likelihood}
...
cap <- paste0(
  "\\label{fig:sensitivity-to-outliers}",
  "Power transformations are affected by outliers. ",
  "Variable $x$ consists of $1000$ values randomly drawn from a normal distribution $\\mathcal{N}(\\mu, 1)$, ",
  ...
  "and even breaks down due to numerical issues for $d > 5 \\cdot 10^4$.")
...
```{=latex}
\begin{equation}
\left\{ \hat{\lambda}^{\text{rYJ}}, \hat{x}_0^{\text{rYJ}} \right\} = \argmax_{\lambda, x_0} \frac{1}{\sum_{i=1}^n w_i} \sum_{i=1}^n w_i \left[ -\frac{1}{2}\log(\sigma^2_w) + (\lambda - 1) \sgn(x_i - x_0) \log(1 + |x_i - x_0|) \right]
\end{equation}
```
...
Other criteria aside from MLE can be used for finding robust transformation parameters, see the Appendix. For the remainder, we will use MLE as the optimisation criterion. 
...
- Set weights based on the z-score of the transformed feature $\phi^{\lambda, x_0} (\mathbf{X})$. After @Raymaekers2021-kq, $z_i = \frac{\phi^{\lambda, x_0}(x_i) - \mu_M}{\sigma_M}$. Here, $\mu_M$ and $\sigma_M$ are robust Huber M-estimates of location and scale of the transformed feature $\phi^{\lambda, x_0} (\mathbf{X})$ [@Huber1981-su].
...
To assess whether the modified power transformations lead to $\lambda$ values that are invariant to shifts, we simulate three different distributions. The first distribution is a normal distribution, as previously shown in Figure \ref{fig:decreased-normality}.
...
We then compute transformation parameter $\hat{\lambda}$ using the original definitions (equations \ref{box-cox-original} and \ref{yeo-johnson-original}) and the shift-sensitive definitions (equations \ref{box-cox-shift} and \ref{yeo-johnson-shift}) for each distribution, after adding a positive value $d$ to each distribution. Here, $d \in [1, 10^6]$.

```{r shifted-distributions, warning=FALSE, message=FALSE, fig.cap=cap}
cap <- paste0(
  "\\label{fig:shifted-distributions}",
...
The result is shown in Figure \ref{fig:shifted-distributions}. For each distribution, transformation parameter $\hat{\lambda}$ varies with $d$ when estimated using the original definitions. In contrast, estimation of $\lambda$ using shift-sensitive power transformations was invariant to $d$.
...
```
We then consider the empirical probability of type I errors: the probability of incorrectly classifying a centrally normal distribution as being non-normal based on the test statistic. Under the assumption that the asymmetric generalised normal distributions are centrally normal after robust transformation, this produces the relationship shown in Figure \ref{tab:empirical-central-normality-type-1-error-rate}. Since for $\kappa \leq 0.80$ error curves are similar, we fix the value of the central portion $\kappa$ to 0.80. This leads to test statistic values listed in Table \ref{tab:empirical-central-normality}, which derive from the type I error rates of the Yeo-Johnson transformation.

```{r empirical-central-normality-type-1-error-rate, warning=FALSE, message=FALSE, fig.cap=cap}
cap <- paste0(
  "\\label{tab:empirical-central-normality-type-1-error-rate}",
...
**MISSING TABLE DATA FOR YEO-JOHNSON**

```{=latex}
\begin{table}
\begin{center}
\label{tab:empirical-central-normality}
\caption{Probability of central normality, based on type I error rates for Box-Cox transformations.}
...
\midrule
0.042 & 0.063 & 0.077 & 0.090 & 0.106 & 0.118 & 0.159 \\
\bottomrule
\end{tabular}
\end{center}
\end{table}
...
In this section, we assess shift sensitive power transformations using real datasets.
...
```{r top-gear, warning=FALSE, message=FALSE, fig.cap=cap, fig.height=2}
cap <- paste0(
  "\\label{fig:top-gear}",

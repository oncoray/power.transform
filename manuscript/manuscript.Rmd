...
- Set weights based on quantiles of the original feature $x$. Quantiles are determined as $p_i = \frac{i - 1/3}{n + 1/3}$, with $i = 1, 2, \ldots n$, with $n$ the number of instances of feature $x$.
...
- Set weights based on the residual error between the z-score of the transformed feature $\phi^{\lambda, x_0} (x)$ and the theoretical z-score from a standard normal distribution: $\left| \left( \phi^{\lambda, x_0}(x_i) - \mu_M)\right) / \sigma_M - F^{-1}_{\mathcal{N}}(p_i) \right|$, with $\mu_M$, $\sigma_M$ and $p_i$ as defined above.

Based on either of the above, weights are set using a weight function. We use the following functions:

- A step function, with $k_1$ as threshold parameter:
```{=latex}
\begin{equation}
w_i =
\begin{cases}
...
- A triangle function (or generalised Huber weight), with $k_1$ and $k_2 \geq k_1$ as threshold parameters:
```{=latex}
\begin{equation}
w_i =
\begin{cases}

...
Considering single process parameters, the choice of learner (Wilcoxon signed rank test: $p < 10^{-8})$) and normalisation method (Wilcoxon signed rank test: $p = 0.03$) had a significant impact (at significance level $p = 0.05$), but transformation method (Friedman test: $p=0.11$) did not.
...
On the scale of normalised ranks ($[0.0, 1.0]$), the overall estimated marginal effect of using a random forest instead of generalised linear model was $0.226$.
The overall marginal effect of using z-standardisation to normalise features was $0.003$.
Transformation methods had the following marginal effects:
$0.002$ for using conventional Yeo-Johnson transformation instead of no transformation;
$-0.006$ for using robust invariant Yeo-Johnson transformation (with or without empirical central normality test) instead of no transformation; 
and $-0.008$ for using robust invariant Yeo-Johnson transformation (with or without empirical central normality test) instead of conventional Yeo-Johnson transformation.
...
We investigated the use of robust invariant power transformation for machine learning, and did not find a major positive or negative effect. 

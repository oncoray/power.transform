...
cap <- paste0(
  ...
  "The step function was parameterised with $\\delta_1 = 0.60$. ",
...
\left\{ \hat{\delta}_1, \hat{\delta}_2 \right\} = \argmin_{\delta_1, \delta_2} \sum_{i=1}^{m_d} \sum_{j=1}^{m_{out}} \left| \lambda_r \left(\mathbf{X}_{i, j}; \delta_1, \delta_2 \right) - \lambda_0 \left(\mathbf{X}_i \right) \right|
\end{equation}
```
...
Location- and scale-invariant power transformations are implemented in the `power.transform` package for R, and is available from [https://github.com/oncoray/power.transform] and the CRAN repository [INSERT LINK HERE]. The manuscript was created using R Markdown and is also available from the power.transform GitHub repository. Data and results for the machine learning experiment are separately available from Zenodo [INSERT LINK HERE].

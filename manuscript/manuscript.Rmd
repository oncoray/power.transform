...
The required algorithms were implemented in the `power.transform` R software package [@Zwanenburg2024-kq].
...
However, in a machine learning experiment of 232 real-world datasets that contained at least one numeric feature, we did not find a meaningful benefit of for -- nor detriment to -- model performance for location- and scale-invariant power transformations.
...
For the latter two datasets, the transformation method does not show significant difference between groups (Friedman test; $p > 0.05$).
```{r include = FALSE, eval = TRUE}
...
While this alternative helps prevent numerical issues -- provided that normalisation does not lead to negative values for Box-Cox transformation -- location- and scale-invariant transformations seem to provide an overall better transformation to normality (Appendix F).

...
The result is shown in Figure \ref{fig:shifted-distributions}. For each distribution, transformation parameter $\hat{\lambda}$ varies with $d_{\text{shift}}$ and $d_{\text{scale}}$ when estimated using the original definitions. In contrast, estimation of $\lambda$ using invariant power transformations was invariant to $d_{\text{shift}}$ and $d_{\text{scale}}$. While the effect of location on the estimated value of $\lambda$ using conventional power transformations is noticeable, scale also affects $\lambda$ for features that are not normally distributed.
The figure also shows that estimated $\lambda$ for Box-Cox power transformations in left-skewed distributions diverges from that found for invariant Box-Cox power transformations.

...
no transformation (residual sum $\sum r_i = 54$, $p=0.56$);
conventional transformation ($\lambda = -0.1$, $\sum r_i = 55$, $\mu_{YJ} = 3.0$, $\sigma_{YJ} = 0.3$, $p=0.01$);
Raymaekers and Rousseeuw's robust adaptation ($\lambda = 0.8$, $\sum r_i = 48$, $\mu_{YJ} = 29$, $\sigma_{YJ} = 15$, $p=0.61$);
location- and scale-invariant transformation ($\lambda = -1.3$, $\sum r_i = 44$, $\mu_{YJ} = 0.5$, $\sigma_{YJ} = 0.1$, $p=0.13$);
and robust location- and scale-invariant transformation ($\lambda = -0.9$, $\sum r_i = 50$, $\mu_{YJ} = 2.0$, $\sigma_{YJ} = 2.3$, $p=0.10$).
...
Figure \ref{fig:experimental-results-outlier-robustness} shows that non-robust transformations fail to produce a centrally normal distribution (empirical central normality test $p = 0.01$ and $p=0.02$ for conventional and invariant transformations, respectively).
Robust transformations produce distributions that are centrally normal (empirical central normality test $p > 0.05$).
...
As a limitation, we did observe several numerical issues for optimisation criteria other than MLE (appendix B).

...
Many statistical and several machine learning methods assume normality of the underlying data. However, numerical features in datasets may strongly deviate from normal distributions, e.g. by being skewed. Power transformations aim to stabilise variance and improve normality of such features [@Bartlett1947-rx, @Tukey1957-rt]. The two most commonly used transformations are that of @Box1964-mz and @Yeo2000-vw. The Box-Cox transformation of a feature value $x_i$ of feature $\mathbf{X}=\left\{x_1, x_2, \ldots, x_n \right\}$ under the transformation parameter $\lambda$ is defined as:
...
\end{equation}
```
The $\lambda$-parameter is typically optimised using maximum likelihood estimation under the assumption that the transformed feature is normally distributed. As noted by Raymaekers and Rousseeuw, this approach is sensitive to outliers, and robust versions of Box-Cox and Yeo-Johnson transformations were devised [@Raymaekers2021-kq].

Power transformation does not guarantee that transformed variables are normally distributed.
Depending on location and scale of a feature and the presence of outliers, power transformation may decrease normality, as shown in Figure \ref{fig:decreased-normality}.
This is problematic when normality of the transformed feature is not checked, e.g. in automated power transformation in machine learning workflows.
Several issues may arise.
First, some machine learning methods assume normality.
Second, a large negative or positive $\lambda$-parameter may lead to numeric issues.
Third, we currently do not have a test to automatically reject poor transformations to normality.
Statistical tests for normality exist, such as the Shapiro-Wilk test [@Shapiro1965-zd].
However, given sufficiently large sample sizes such tests can detect trivial deviations from normality.

```{r decreased-normality, echo=FALSE, fig.cap=cap, fig.height=2.0, warning=FALSE}
cap <- paste0(
  ...
  "and breaks down due to numerical issues for $\\mu > 5 \\cdot 10^3$. ",
  "Here, scale did not affect Box-Cox and Yeo-Johnson transformations of variables drawn from normal distributions."
)
...
Subsequently, we define weighted location and scale invariant transformations and weighting methods that allow for defining robust transformations.
...
\end{equation}
```
where $\mu$ and $\sigma^2$ are the mean and variance of the Box-Cox transformed feature $\phi_{\text{BC}}^{\lambda, x_0, s} (\mathbf{X})$, respectively. Similarly, the invariant Yeo-Johnson log-likelihood function is:
...
Weights may be set using several methods. Here, we investigate three:
...
Weights are set using a weighting function for all three methods. We will investigate the following functions, where $\dot{x}_i$ can be either $p^{*}_i=2 \left( p_i - 0.5\right)$ (so that the quantiles are zero-centred), $z_i$ or $r_i$:
...
In this section we used simulated data to assess invariance to location and scale of the proposed power transformations, weighting for robust transformations, and to develop the empirical central normality test.
...
To assess whether the proposed power transformations lead to values of $\lambda$ that are invariant to location and scale of the distribution, we simulated three different distributions. The first distribution was a normal distribution, as previously shown in Figure \ref{fig:decreased-normality}.
...
We then computed transformation parameter $\lambda$ using the original definitions (equations \ref{eqn:box-cox-original} and \ref{eqn:yeo-johnson-original}) and the location and scale invariant definitions (equations \ref{eqn:box-cox-invariant} and \ref{eqn:yeo-johnson-invariant}) for each distribution.
...
For each distribution, transformation parameter $\lambda$ varied with $d_{\text{shift}}$ and $d_{\text{scale}}$ when estimated using the original definitions.
The effect of location on the estimated $\lambda$ value for conventional power transformations is noticeable for all three distributions.
However scale only affected $\lambda$ for features that were sampled from a skewed distribution.
In contrast, estimation of $\lambda$ using invariant power transformations was invariant to both $d_{\text{shift}}$ and $d_{\text{scale}}$. 

## Robust transformations

Outliers may be present in data and affect estimation of power transformation parameters.
...
We proposed three weighting methods: probabilities of the empirical distribution of the original feature, the z-score of the transformed feature values, or the residual error between the z-score of the transformed feature values and their expected z-score based on the normal distribution.
The weight itself is then assigned to each instance using one of three weighting functions: step, triangle and tapered cosine.
The weighting functions have one (step) or two (triangle, tapered cosine) parameters.
Here we investigate how these parameters should be set to find robust transformation parameter $\lambda_r$.

We randomly drew $m_d=100$ asymmetric generalised normal distributions.
...
This was repeated $m_{out} = 10$ times, with outlier fractions regularly spaced in $[0.00, 0.10]$.
...
Outlier values randomly replaced values in $\mathbf{X}_i$ to create $\mathbf{X}_{i,j}$.

To find the optimal values for the weighting function parameters $k_1$ and $k_2$ (if applicable), we minimised the absolute difference between the $\lambda_{r}$ parameter obtained for transformation in the presence of outliers, and the $\lambda_0$ parameter obtained using the non-robust transformations in absence of outliers:

```{=latex}
\begin{equation}
\label{eqn:minimisation-weighting}
\left\{ \hat{k}_1, \hat{k}_2 \right\} = \argmin_{k_1, k_2} \sum_{i=1}^{m_d} \sum_{j=1}^{m_{out}} \left| \lambda_r \left(\mathbf{X}_{i, j}; k_1, k_2 \right) - \lambda_0 \left(\mathbf{X}_i \right) \right|
\end{equation}
```
...
\begin{table}
\begin{center}
\caption{Optimal weighting parameters and corresponding loss for \textbf{location- and scale-invariant Box-Cox} power transformations. $p^{*}$ indicates use of the empirical distribution of feature values, $z$ the z-score of the transformed feature values, and $r$ the residual error between the z-score of transformed feature values and the expected z-score according to the normal distribution. The \textit{initial} column shows the starting parameter value for the optimisation process, with the corresponding boundary values in the \textit{limits} column. The {optimal} column shows the optimal parameter values. The \textit{loss} column shows the loss achieved by each method, under optimised parameters. Lower loss indicates better robustness against outliers.}
...
Invariant power transformations are intended to yield improved transformations to normality
in the presence of large shifts in location, scales far from unity, or both.
Above, this characteristic was assessed using simulations. Below, this characteristic
will be evaluated using examples from real datasets.

### Age of patients with lung cancer

A common variable in health-related datasets is age.
...
Applying conventional and invariant Yeo-Johnson transformations to patient age yielded the following results, see Figure \ref{fig:lung-cancer-age}:
no transformation (residual sum $\sum r_i = 16.5$);
...
```{r lung-cancer-age, warning=FALSE, message=FALSE, fig.cap=cap}
cap <- paste0(
  "Quantile-quantile plots for age of patients with lung cancer, expressed in years. ",
  ...
  "the non-robust and robust invariant transformations."
)

out <- .plot_lung_cancer_age(plot_theme = plot_theme)
out$plot
```

Location- and scale-invariant transformation leads to a lower overall residual sum, indicating a better transformation.
Moreover, conventional transformations inflated the mean $\mu_{YJ}$ and standard deviation $\sigma_{YJ}$ of the age variable after transformation.
...
Applying conventional and invariant Yeo-Johnson transformations to patient age yielded the following results, see Figure \ref{fig:penguin-body-mass}:
no transformation (residual sum $\sum r_i = 48.0$);
conventional transformation ($\lambda = 0.5$, $\sum r_i = 32.2$, $\mu_{YJ} = 2.102$, $\sigma_{YJ} = 4 \cdot 10^{-3}$);
Raymaekers and Rousseeuw's robust adaptation ($\lambda = 0.5$, $\sum r_i = 32.2$, $\mu_{YJ} = 2.101$, $\sigma_{YJ} = 4 \cdot 10^{-3}$);
...
```{r penguin-body-mass, warning=FALSE, message=FALSE, fig.cap=cap}
cap <- paste0(
  "Quantile-quantile plots for penguin body mass, measured in grams. ",
  "The left panel shows the quantile-quantile plot for the original data. ",
  "The middle panel shows the quantile-quantile plots for data transformed using ",
  "the original Yeo-Johnson transformation, and Raymaekers and Rousseeuw's robust adaptation \\cite{Raymaekers2021-kq}. ",
  "The right panels shows the quantile-quantile plots for data transformed using ",
...
out <- .plot_penguin_body_mass(plot_theme = plot_theme)
out$plot
```
Location- and scale-invariant transformation produced a lower overall residual sum, indicating a better transformation.
Moreover, conventional transformations led to low standard deviation $\sigma_{YJ}$ of the body mass variable after transformation.
...
Applying conventional and invariant Yeo-Johnson transformations to latitude yielded the following results, see Figure \ref{fig:ames-latitude}:
...
```{r ames-latitude, warning=FALSE, message=FALSE, fig.cap=cap}
cap <- paste0(
  "Quantile-quantile plots for latitude coordinate in a housing dataset. ",
  "The left panel shows the quantile-quantile plot for the original data. ",
  "The middle panel shows the quantile-quantile plots for data transformed using ",
  "the original Yeo-Johnson transformation, and Raymaekers and Rousseeuw's robust adaptation \\cite{Raymaekers2021-kq}. ",
  "The right panels shows the quantile-quantile plots for data transformed using ",
...
out <- .plot_ames_latitude(plot_theme = plot_theme)
out$plot
```
...
TranConventional transformations had high values for the $\lambda$ parameter.
...
cap <- paste0(
  "Quantile-quantile plots for fuel consumption data in the Top Gear dataset. ",
  "The dataset contains three strong outliers for highly fuel-efficient vehicles.",
  "The left panel shows the quantile-quantile plot for the original data. ",
  "The empirical central normality test indicates that the original data do not deviate significantly from central normality ($p=0.76$).",
  "The middle panel shows the quantile-quantile plots for data transformed using ",
  "the original Yeo-Johnson transformation, and Raymaekers and Rousseeuw's robust adaptation \\cite{Raymaekers2021-kq}. ",
  "For these, the empirical central normality test yields $p=0.01$, and $p=0.55$, respectively. ",
  "The right panels shows the quantile-quantile plots for data transformed using ",
  "the non-robust and robust invariant transformations. ",
  "For these, the empirical central normality test yields $p=0.03$, and $p=0.86$, respectively. ",
...
The Top Gear dataset contains data on 297 vehicles that appeared on the BBC television show *Top Gear*. Within this dataset, the fuel consumption feature contains outliers due to highly fuel-efficient vehicles. Figure \ref{fig:top-gear} shows that the outliers cause non-robust transformations to fail to transform the data to a centrally normal distribution (empirical central normality test $p = 0.01$ and $p=0.03$ for conventional and invariant transformations, respectively). Robust transformations produce distributions that are centrally normal (empirical central normality test $p > 0.05$).

### Maximum arterial wall thickness in an ischemic stroke dataset

```{r ischemic-stroke, warning=FALSE, message=FALSE, fig.cap=cap}
cap <- paste0(
  "Quantile-quantile plots for the maximum arterial wall thickness in the ischemic stroke dataset [@Romano2022-gq]. ",
  "This data contains two strong outliers. ",
  "The left panel shows the quantile-quantile plot for the original data. ",
  "The empirical central normality test indicates that the original data do not deviate significantly from central normality ($p=0.56$).",
  "The middle panel shows the quantile-quantile plots for data transformed using ",
  "the original Yeo-Johnson transformation, and Raymaekers and Rousseeuw's robust adaptation [@Raymaekers2021-kq]. ",
  "For these, the empirical central normality test yields $p=0.01$, and $p=0.61$, respectively. ",
  "The right panels shows the quantile-quantile plots for data transformed using ",
  "the non-robust and robust location- and scale-invariant transformations. ",
  "For these, the empirical central normality test yields $p=0.02$, and $p=0.10$, respectively. ",
  "Samples with observed quantiles below $-3.0$ or above $3.0$ are indicated by crosses."
)

out <- .plot_ischemic_stroke(
  plot_theme = plot_theme
)
...
Figure \ref{fig:ischemic-stroke} show that non-robust transformation fail to produce a centrally normal distribution (empirical central normality test $p = 0.01$ and $p=0.02$ for conventional and invariant transformations, respectively). Robust transformations produce distributions that are centrally normal (empirical central normality test $p > 0.05$).
...
Considering single process parameters, the choice of learner (Wilcoxon signed rank test: $p < 10^{-8})$) and normalisation method (Wilcoxon signed rank test: $p = 0.007$) had a significant impact (at significance level $p = 0.050$), but transformation method (Friedman test: $p = 0.054$) did not.
...
cap <- paste0(
  "Estimated marginal effect of learners, normalisation and transformation methods on ranked model performance scores in 22800 machine learning experiments on 285 datasets. ",
  ...
  "ECTN: empirical central normality test."
)
...
Robust location- and scale-invariant transformations form a suitable replacement for their conventional counterparts.
They demonstrated robustness against outliers and prevent inaccurate transformations and potential numerical issues due to location and scale of the data distribution.
This is particularly relevant for automated data processing, where such issues may go unnoticed.
However, we did not find a measurable benefit for -- nor detriment to -- model performance in a machine learning experiment of 232 real-world datasets that contained at least one numeric variable.
One reason may be that numeric features with large location shifts ($|\mu| > 1000.0$) were uncommon.
Of the 4886 numeric variables in the 232 datasets, 266 (5%) variables in 34 datasets had large location shifts, of which 200 appeared in just 2 datasets.

Robust location- and scale-invariant transformations are realised by simultaneously optimising three parameters, i.e. transformation parameter $\lambda$, shift parameter $x_0$ and scale parameter $s$, using maximum likelihood optimisation.
...
First, in our experimental results we focussed on the Yeo-Johnson transformation because of its ability to handle variables with negative values, and did not consider the Box-Cox transformation.
Likewise, we focussed on maximum likelihood estimation to optimise parameters, and did not investigate other metrics to the same extent, due to lack of demonstrable benefit (Appendix B).

# Conclusion

Robust location- and scale-invariant power transformations allow for automated power transformation of numerical variables/

# Code availability

Location- and scale-invariant power transformations are implemented in the `power.transform` package. This manuscript was created using R Markdown, and is available from [https://github.com/alexzwanenburg/power.transform].

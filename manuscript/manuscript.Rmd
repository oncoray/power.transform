...
Minimisation is performed using the subplex algorithm implemented in the *nloptr* package for R [@Rowan1990-sn; @Ypma2022-tc].
...
\begin{table}
\begin{center}
\caption{Optimal weighting parameters and corresponding loss. $p^{*}$ indicates use of the empirical distribution of feature values, $z$ the z-score of the transformed feature values, and $r$ the residual error between the z-score of transformed feature values and the expected z-score according to the normal distribution. The \textit{initial} column shows the starting parameter value for the optimisation process, with the corresponding boundary values in the \textit{limits} column. The {optimal} column shows the optimal parameter values. The \textit{loss} column shows the loss achieved by each method, under optimised parameters. Lower loss indicates better robustness against outliers.}
...
$z$ (step)               & 1.50 & $(0, 10]$ & 1.03 & ---  & ---       & ---   & 331 \\
$z$ (triangle)           & 1.50 & $(0, 10]$ & 2.79 & 3.00 & $(0, 10]$ & 4.12  & 768 \\
$z$ (tapered cosine)     & 1.50 & $(0, 10]$ & 2.69 & 3.00 & $(0, 10]$ & 4.21  & 772 \\
$r$ (step)               & 0.15 & $(0, 10]$ & 0.05 & ---  & ---       & ---   & 1957 \\
$r$ (triangle)           & 0.15 & $(0, 10]$ & 9.96 & 1.00 & $(0, 10]$ & 10.00 & 483 \\
$r$ (tapered cosine)     & 0.15 & $(0, 10]$ & 9.97 & 1.00 & $(0, 10]$ & 10.00 & 484 \\
\bottomrule
\end{tabular}
\end{center}
\end{table}
...
```{r outlier-values, warning=FALSE, message=FALSE, fig.cap=cap}
cap <- paste0(
  "CHECK. ")
        ...
        for(ii in seq_len(100)){
          for(k in k_range){
            ...
            for(method in c("yeo_johnson")){
              for(weight_method in c("none", "original_cosine", "transformed_step", "transformed_cosine", "residual_step", "residual_cosine")){
                
                k1 <- k2 <- NULL
                
                # Optimal parameter values from the previous experiment.
                if(weight_method == "original_cosine"){
                  k1 <- 0.89
                  k2 <- 0.93
...
data$method <- factor(
  x = data$method,
  levels = c("yeo_johnson"),
  labels = c("Yeo-Johnson"))
data$weight_method <- factor(
  x = data$weight_method,
  levels = c("none", "original_cosine", "transformed_step", "transformed_cosine", "residual_step", "residual_cosine"),
  labels = c("non-robust", "emp. prob. (tap. cos.)", "z-score (step)", "z-score (tap. cos.)", "residual (step)", "residual (tap. cos.)"))
...
top_group <- c("non-robust", "emp. prob. (tap. cos.)", "z-score (step)")
bottom_group <- c("z-score (tap. cos.)", "residual (step)", "residual (tap. cos.)")

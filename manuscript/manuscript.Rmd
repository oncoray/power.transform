...
Power transformation does not guarantee that transformed features are normally distributed.
...
Also, large negative or positive $\lambda$-parameters may lead to numeric issues in any subsequent computations.
...
cap <- paste0(
  "Effect of location shift, scale and outliers on estimation of Box-Cox and Yeo-Johnson transformation parameter $\\lambda$. ",
...
\end{equation}
```
where $\mu$ and $\sigma^2$ are the mean and variance of the Box-Cox transformed feature $\phi_{\text{BC}}^{\lambda, x_0, s} (\mathbf{X})$, respectively.
...
As indicated by @Raymaekers2021-kq, the general aim of power transformations should be to transform non-outlier data to normality, i.e. achieve *central normality*.
...
@Raymaekers2021-kq achieve this through weighted maximum likelihood estimation.
...
Compared to @Raymaekers2021-kq, these log-likelihood functions includes additional terms to accommodate estimation of $x_0$ and $s$. The weighted location- and scale-invariant Box-Cox log-likelihood function is:
...
All weighting functions share the characteristic that for $\left| \dot{x}_i \right|< k_1$, instances are fully weighted, i.e. when $k_1 > 0$ the weighting functions are symmetric window functions with a flat top.
The triangle and tapered cosine functions then gradually down-weight instances with $k_1 \leq \left| \dot{x}_i \right| \leq k_2$, and assign no weight to instances $\left| x_i \right| > k_2$.
...
- The weighting function uses empirical probabilities of the distribution of the original feature $\mathbf{X}$.
  Probabilities are determined as $p_i = \frac{i - 1/3}{n + 1/3}$, with $i = 1, 2, \ldots n$, with $n$ the number of instances of feature $\mathbf{X}$.
...
- The weighting function uses the z-score of the transformed feature $\phi^{\lambda, x_0, s} (\mathbf{X})$.
  After @Raymaekers2021-kq, $z_i = \frac{\phi^{\lambda, x_0, s}(x_i) - \mu_M}{\sigma_M}$.
...
- The weighting function uses the residual error between the z-score of the transformed feature $\phi^{\lambda, x_0, s} (\mathbf{X})$ and the theoretical z-score from a standard normal distribution: $r_i =\left| \left( \phi^{\lambda, x_0, s}(x_i) - \mu_M)\right) / \sigma_M - F^{-1}_{\mathcal{N}}(p_i) \right|$, with $\mu_M$, $\sigma_M$ and $p_i$ as defined above.
  Then $\dot{x}_i = r_i$.
...
Moreover, conventional transformations inflated the mean $\mu_{YJ}$ and standard deviation $\sigma_{YJ}$ of the age feature after transformation.
...
However, we did not find a measurable benefit for -- nor detriment to -- model performance in a machine learning experiment of 232 real-world datasets that contained at least one numeric feature.
...
Robust location- and scale-invariant power transformations allow for automated power transformation of numerical features.

# Code availability

Location- and scale-invariant power transformations are implemented in the `power.transform` package for R, and is available from [https://github.com/oncoray/power.transform] and the CRAN repository. The manuscript was created using R Markdown and is also available from the power.transform GitHub repository.

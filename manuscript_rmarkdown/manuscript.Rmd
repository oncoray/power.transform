...
\begin{definition}[Centrally normal distribution]

A centrally normal distribution is any distribution whose quantile function is 
...
If the sum of residual errors of the central portion of a sequence is 0, i.e.,
$\sum_{r_j \in \mathbf{R}_{\text{c}}} r_j = 0$, 
the values of the elements in the central portion of that sequence follow a 
centrally normal distribution. 

\end{corollary}

By the Glivenko-Cantelli theorem, if sequence $\mathbf{X}$ was sampled from a
normal distribution $\mathcal{N}(\mu, \sigma)$,
$\lim_{n \rightarrow \infty} \sum_{r_j \in \mathbf{R}_{\text{c}}} r_j = 0$,
i.e. an infinite sequence sampled from a normal distribution has a central portion
with values sampled from a centrally normal distribution.

\begin{definition}[Central normality test]
The null-hypothesis $\mathcal{H}_0$ is that the sequence $\mathbf{X}$ was sampled
from a population that has a centrally normal distribution.

The alternative hypothesis is that the sequence $\mathbf{X}$ was sampled from a
population that lacks a centrally normal distribution.
\end{definition}
...
$\tau_{n, \kappa} \geq \tau_{\alpha, n, \kappa, \zeta, \text{critical}}$,
with $\zeta$ the population estimate of the rate of outliers. Critical test
values are determined in the [Simulation](#simulation) section.
...
the significance level $\alpha$, sequence length $n$, central portion $\kappa$
and estimated outlier rate $\zeta$.
...
Outliers were inserted by randomly replacing elements of an ordered sequence with probability 
$\zeta \in \left\{0.0, 0.01, 0.02, 0.05, 0.10, 0.15, 0.20 \right\}$. 
...
For each sequence we then computed $\tau_{\kappa}$ for $\kappa \in \left\{0.60, 0.70, 0.80, 0.90, 0.95, 1.00\right\}$.
...
Figure \ref{fig:empirical-central-normality-kappa} shows the critical test static
$\tau_{\alpha = 0.05, n, \kappa, \zeta}$
as a function of $n$ for different values of $\kappa$ and $\zeta$.
With decreasing $\kappa$, the test statistic curve decreases.
The curves for $\kappa = 0.60$, $\kappa = 0.70$ and $\kappa = 0.80$ are similar,
whereas curves for $\kappa \geq 0.90$ were affected by outliers.

We use $\kappa = 0.80$ and $\zeta = 0.10$ for the empirical central normality tests 
...
$\kappa = 0.80$ and $\zeta = 0.10$ are shown in Table 
...
cap <- paste0(
  "Critical test statistic $\\tau_{\\alpha = 0.05, n, \\kappa, \\zeta}$ of the empirical ",
  "central normality test as function of $n$ for several values of central portion $\\kappa$ ",
  "and estimated outlier rates $\\zeta$. ",
  "emp: empirical"
)
...
($\kappa = 0.80$ and $\zeta \in \left\{0.00, 0.05, 0.10\right\}$) and compare
...
Each sequence was additionally randomly populated with outliers at 5% and 10% rates, as earlier.
...
The empirical test for central normality differs from other tests for normality, 
...
Compared to the central normality test and the Shapiro-Wilk test, the empirical central
normality test remains consistent in the presence of outliers, although the former
tests are more powerful when outliers are absent.

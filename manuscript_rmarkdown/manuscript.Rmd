...
the significance level $\alpha$, sequence length $n$ and central portion $\kappa$.
...
we randomly sampled $m_d = 30000$ ordered sequences $\mathbf{X}$ from $\mathcal{N}(0,1)$.
For each sequence we then computed $\tau_{\kappa}$ for 
...
these to the Shapiro-Wilk test for normality, we randomly drew another 10000 sequences from a
central normal distribution for each 
$n \in \left\{\lfloor 10^\nu \rfloor | \nu \in \left\{0.7500, 0.8125, \ldots, 3.0000 \right \} \right\}$.

Each sequence was created as follows: first $n$ values are sampled from $\mathcal{N}(0, 1)$.
Then, the elements outside the central portion of the sequence (i.e. $\mathbf{X} \setminus \mathbf{X}_{\text{c}}$) are replaced.
...
hypothesis that sequences were derived from a population with a (central) normal distribution.
The null hypothesis was rejected if $p \leq 0.05$.
The type I error rate was determined by computing the fraction of sequences rejected this way.
...
Three datasets with 100000 sequences each were created to assess transformation
...
- A \textit{dirty} dataset with each sequence drawn from $\mathcal{AGN}\left(\mu = 0, \sigma = 1/\sqrt{2}, \alpha, \beta \right)$,
  ...
  and shape parameter $\beta \sim U\left(1.00, 5.00 \right)$.
  The sequences are drawn from a range of skewed, unimodal, but not necessarily 
  normal, distributions, to reflect less \textit{clean} data then that presented in the first dataset.
  
- A \textit{shifted} dataset with each sequence drawn from $\mathcal{AGN}\left(\mu = 1000, \sigma = 1/\sqrt{2} , \alpha, \beta \right)$,
  with randomly drawn skewness parameter $\alpha \sim U\left(0.01, 0.99\right)$ 
  and shape parameter $\beta \sim U\left(1.00, 5.00 \right)$.
  The generating process creates sequences similar to that in the \textit{dirty}
...
transformation method was computed over all 100000 sequences in each dataset.
...
The shifted dataset also contains sequences sampled from asymmetric generalised normal distributions, but centred at 1000.
...
We introduced an empirical test for central normality to assess whether sequences
...
This work has the following limitation: We observed several numerical stability

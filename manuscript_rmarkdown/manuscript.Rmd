...
Figure \ref{fig:empirical-central-normality-error_rate} shows that when the estimated outlier rate 
for the empirical central normality test matches the outlier rate in the data,
the test yields an accurate Type I error rate.
The Shapiro-Wilk test is overly conservative in the presence of any outliers. 
Empirical central normality tests where the estimated outlier rate is lower
than the actual outlier rate are likewise conservative, but to a lesser degree. 
Empirical central normality tests where the estimated outlier rate is higher 
than the actual outlier rate are optimistic.

```{r empirical-central-normality-error_rate, warning=FALSE, message=FALSE, fig.cap=cap, fig.height=2.5}
cap <- paste0(
  "Observed type I error rates for central normality tests in the presence of data sampled from normal distributions ",
  "without outliers, or with a 5% or 10% outlier rate. ",
  "The empirical central normality (ECN) test used $\\kappa = 0.80$, and was further parameterised by the ",
  "estimated outlier rate $\\zeta = 0.00$ (ECN (0 %)), $\\zeta = 0.05$ (ECN (5 %)) and $\\zeta = 0.10$ (ECN (10%))."  
)

...
parameters: the central portion $\kappa$ and outlier rate $\zeta$ as fixed parameters,
and test statistic $\tau_{\text{ecn}}$. We will first define the central portion
$\kappa$.
...
Additionally, outliers were inserted by randomly replacing each element of an ordered sequence with a probability $\zeta$.
We then computed the critical test statistic values for the empirical central normality test.
For each sequence we then computed $\tau_{n, \kappa, \zeta}$ for 
$\kappa \in \left\{0.60, 0.70, 0.80, 0.90, 0.95, 1.00\right\}$ and
...
Figure \ref{fig:empirical-central-normality-kappa} shows $\tau_{\alpha = 0.05, n, \kappa, \zeta}$
as a function of $n$ for different values of $\kappa$ and $\zeta$. With decreasing
...
we used $\kappa = 0.80$. Critical statistic values for $\kappa = 0.80$ and $\zeta = 0.10$ are shown in Table 
...
Figure \ref{fig:empirical-central-normality-error_rate} shows that the empirical central normality test had the expected type I error rate for

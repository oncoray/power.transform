...
In these examples location- and scale-invariant transformations have similar or lower residual errors compared to errors resulting from normalisation prior to transformation.

```{r include = FALSE, eval = FALSE}

data <- .assess_standardisation_before_transformation()
data <- data[transformation %in% c("none", "conventional", "conventional (z-score norm.)", "conventional (robust scaling)", "invariant"), mget(c("residuals", "transformation", "dataset"))]
data[, "residuals" := round(residuals, digits = 1L)]
data.table::dcast(data, dataset ~ transformation, value.var = "residuals")
```
```{=latex}
\begin{table}
\begin{center}

...
location- and scale-invariant transformation ($\lambda = -0.7$, $\sum r_i = 44$, $\mu_{BC} = 0.6$, $\sigma_{BC} = 0.2$, $p=0.02$);
...
Raymaekers and Rousseeuw's robust adaptation ($\lambda = 1.1$, $\sum r_i = 127$, $\mu_{BC} = 5.5$, $\sigma_{BC} = 12$, $p=0.60$);
location- and scale-invariant transformation ($\lambda = -1.0$, $\sum r_i = 28$, $\mu_{BC} = 0.7$, $\sigma_{BC} = 0.1$, $p=0.01$);
and robust location- and scale-invariant transformation ($\lambda = 0.5$, $\sum r_i = 56$, $\mu_{BC} = 2.2$, $\sigma_{BC} = 1.4$, $p=0.35$).
...
\begin{definition}[Empirical central normality test]
$\mathcal{H}_0: \sum_{r_j \in \mathbf{R}_{\text{c}}}r_j = 0$

$\mathcal{H}_1: \sum_{r_j \in \mathbf{R}_{\text{c}}}r_j > 0$
\end{definition}
...
\begin{definition}[Empirical central normality test statistic]
Let $\alpha$ be the significance level for the empirical central normality test.
Let $m$ be the number of elements of $\mathbf{R}_{\text{c}}$

The test statistic is then $\tau_{\text{ecn}, \alpha, n, \kappa} = \frac{1}{m}\sum_{r_j \in \mathbf{R}_{\text{c}}}r_j$.

\end{definition}

Critical values for the test statistic of the empirical central normality test 
are found in Table \ref{tab:empirical-central-normality-critical-statistic}. 
The null-hypothesis should be rejected at significance level $\alpha$ if 
$\tau_{\text{ecn}, \alpha, n, \kappa} \geq \tau_{\text{ecn}, \alpha, n, \kappa, \text{critical}}$.

```{r include = FALSE, eval = FALSE}
# This writes the lookup table for critical test statistic.
load(file = "./R/sysdata.rda")

gof_lookup_table <- .get_test_statistic_lookup_table(
...
\begin{table}
\begin{center}
\caption{Critical values of test statistic $\tau_{\text{ecn},\alpha, n}$ for
empirical central normality at $\kappa = 0.70$, as a function of significance 
level $\alpha$ and number of instances $n$. The presented values should be divided by $100$.}
...
\midrule
5     & 25.28 & 22.17 & 17.46 & 13.15 & 10.48 & 8.32 & 6.62 & 4.93 & 2.24 \\
10    & 21.11 & 18.16 & 13.66 & 10.14 &  8.67 & 7.59 & 6.74 & 5.89 & 4.12 \\
20    & 14.93 & 12.96 &  9.97 &  7.71 &  6.77 & 6.11 & 5.56 & 5.02 & 3.97 \\
50    &  9.69 &  8.49 &  6.59 &  5.20 &  4.63 & 4.22 & 3.91 & 3.58 & 2.99 \\
100   &  6.92 &  6.06 &  4.71 &  3.73 &  3.32 & 3.04 & 2.83 & 2.59 & 2.20 \\
200   &  4.89 &  4.29 &  3.35 &  2.66 &  2.37 & 2.17 & 2.02 & 1.87 & 1.60 \\
500   &  3.12 &  2.73 &  2.12 &  1.69 &  1.51 & 1.39 & 1.29 & 1.19 & 1.02 \\
1000  &  2.19 &  1.92 &  1.50 &  1.20 &  1.07 & 0.98 & 0.92 & 0.85 & 0.73 \\
2000  &  1.56 &  1.37 &  1.06 &  0.85 &  0.76 & 0.70 & 0.65 & 0.60 & 0.52 \\
5000  &  0.99 &  0.86 &  0.67 &  0.54 &  0.48 & 0.44 & 0.41 & 0.38 & 0.32 \\
10000 &  0.69 &  0.61 &  0.48 &  0.38 &  0.34 & 0.31 & 0.29 & 0.27 & 0.23 \\
\bottomrule
\end{tabular}
\end{center}
\end{table}
...
\midrule
none & 0.685 & 0.558 & 0.764 & 0.354 & 0.030 \\
conventional & 0.886 & 0.010 & 0.012 & 0.317 & 0.381 \\
conventional (z-score norm.) & 0.842 & 0.036 & 0.011 & 0.075 & 0.378 \\
conventional (robust scaling) & 0.855 & 0.002 & 0.007 & 0.077 & 0.416 \\
Raymaekers-Rousseeuw & 0.886 & 0.610 & 0.553 & 0.296 & 0.381 \\
Raymaekers-Rousseeuw (z-score norm.) & 0.685 & 0.632 & 0.812 & 0.075 & 0.378 \\
Raymaekers-Rousseeuw (robust scaling) & 0.826 & 0.609 & 0.813 & 0.077 & 0.434 \\
invariant & 0.907 & 0.134 & 0.026 & 0.049 & 0.549 \\
robust invariant & 0.781 & 0.102 & 0.583 & 0.085 & 0.717 \\
\bottomrule
\end{tabular}
\end{center}
\end{table}

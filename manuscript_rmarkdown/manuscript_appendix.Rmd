...
Let $\alpha$ be the significance level for the central normality test.
Let $m$ be the number of elements of $\mathbf{R}_{\text{c}}$

The test statistic is then $\tau_{\alpha, n, \kappa} = \frac{1}{m}\sum_{r_j \in \mathbf{R}_{\text{c}}}r_j$.

\end{definition}

Critical values for the test statistic of the central normality test 
are found in Table \ref{tab:central-normality-critical-statistic}. These are 
determined by repeatedly and randomly sampling data (i.e. $\mathbf{X}$) from
a normal distribution $\mathcal{N}(0,1)$. The null-hypothesis should be rejected
at significance level $\alpha$ if 
$\tau_{\alpha, n, \kappa} \geq \tau_{\alpha, n, \kappa, \text{critical}}$.

The empirical central normality test is a central normality test, in the
definition provided above. However, critical values of the test statistic are
not determined directly from data drawn randomly from $\mathcal{N}(0,1)$.
Rather, after $\mathbf{X}$ is drawn, 10% of the values are randomly replaced by
random outlier values. The resulting distribution is centrally normal, but
not fully normal.

Critical values for the empirical variant of the central normality test are
